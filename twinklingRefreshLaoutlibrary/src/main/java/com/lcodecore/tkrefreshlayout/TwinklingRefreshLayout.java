...
public class TwinklingRefreshLayout extends RelativeLayout implements PullListener, NestedScrollingChild {
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        if (visibleItemCount == 0 || getTitle() == null) {
            return;
        }
        if (firstVisibleItem == 0) {
            View view1 = view.getChildAt(0);
            getTitle().setTranslationY(view1.getY());
            setY(0);
        } else {
            setY(0);
            getTitle().setTranslationY(-getTitle().getHeight());
        }
    }
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 3; i < getChildCount(); i++) {
            ...
            if (mChildView instanceof ListView || mChildView instanceof RecyclerView) {
                break;
            }
        }
        cp.init();
...
//        cp.startLoadMore();
    ...
    public class CoContext {
        ...
        public void setTargetViewTranslationY(float translationY) {
            mChildView.setTranslationY(translationY);
            if (getTitle() == null) {
                return;
            }
            if (translationY > 0) {
                getTitle().setTranslationY(translationY);
            } else {
                if (getY() == 0) {
                    setY(getTitle().getY());
                }
                getTitle().setTranslationY(getY() + translationY);
            }
        }
